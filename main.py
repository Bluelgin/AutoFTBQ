...
APP_NAME = "AutoFTBQ"; VERSION = "1.0.0"; AUTHOR = "Taki"
    ...
    def __init__(self, parent, mods):
        ...
        try:
            ...
            for i, m in enumerate(mods):
                ...
                self._selected_indices.add(i)  # All selected by default
...
class App:
    ...
    def _on_startup_checked(self, ok):
        if ok: self.set_info(t("ready"), "info")
        else: self.set_info(t("deps_missing"), "error"); self.root.after(100, lambda: messagebox.showwarning(t("dep_error_title"), AI_IMPORT_ERROR))
    ...
    def _check_config_ready(self):
        ...
        if api and mf and os.path.isdir(mf) and AI_AVAILABLE and self.selected_mods:
            self.generate_btn.config(state=tk.NORMAL, bg="#4caf50", activebackground="#388e3c")
            self.set_info(t("config_ready"), "success")
        else:
            self.generate_btn.config(state=tk.DISABLED, bg="#cccccc", activebackground="#cccccc")
            if not AI_AVAILABLE: self.set_info(t("deps_missing"), "error")
            elif not mf: self.set_info(t("select_mod"), "warning")
            elif not api: self.set_info(t("enter_api"), "warning")
            else: self.set_info(t("ready"), "info")
    ...
    def _build_ui(self):
        ...
        # API Key
        sec = tk.Frame(mf, bg="#ffffff"); sec.pack(fill=tk.X, pady=(0, 15))
        self.api_label = tk.Label(sec, font=("Microsoft YaHei", 12, "bold"), fg="#333333", bg="#ffffff")
        self.api_label.pack(anchor=tk.W)
        self.api_desc_label = tk.Label(sec, font=sf, fg="#aaaaaa", bg="#ffffff")
        self.api_desc_label.pack(anchor=tk.W, pady=(2, 6))
        row = tk.Frame(sec, bg="#ffffff"); row.pack(fill=tk.X)
        ...
        # Mod Folder row
        sec2 = tk.Frame(mf, bg="#ffffff"); sec2.pack(fill=tk.X, pady=(0, 15))
    ...
    def _on_save_config(self):
        if not self.api_key_var.get().strip(): messagebox.showwarning("Warning", t("enter_api")); return
    ...
    def _on_generate(self):
        ...
        if not api_key: messagebox.showwarning("Warning", t("enter_api")); return
        ...
        threading.Thread(target=self._generate_thread, args=(api_key, list(self.selected_mods), LANG), daemon=True).start()

    def _generate_thread(self, api_key, selected_mods, lang):
        try:
            mf = self.mod_folder_var.get().strip()
            out = generate_quest_book(api_key=api_key, selected_mods=selected_mods,
                                      mod_folder=mf, progress_callback=self._on_progress, lang=lang)
